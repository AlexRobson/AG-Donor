...
function sidebar_login_options_init() {
	...
	foreach($sidebar_login_options as $section) {
		foreach($section[1] as $option) {
			...
			register_setting( 'wp-sidebar-login', $option['name'] );
		}
	}
...
function sidebar_login_options_add_page() {
	add_options_page(__('WP Sidebar Login','sblogin'), __('WP Sidebar Login','sblogin'), 'manage_options', 'wp-sidebar-login', 'sidebar_login_options');
}
...
function sidebar_login_options() {
	...
	<div class="wrap">
		...
		<?php screen_icon(); echo "<h2>" .__( 'WP Sidebar Login Options','sblogin') . "</h2>"; ?>
		
		<form method="post" action="options.php">
		
			<?php settings_fields( 'wp-sidebar-login' ); ?>
				...
				foreach($section[1] as $option) {
					...
					switch ($option['type']) {
						...
						case "textarea" :
							...
							?><textarea id="<?php echo $option['name']; ?>" class="large-text" cols="30" rows="10" name="<?php echo $option['name']; ?>" placeholder="<?php if (isset($option['placeholder'])) echo $option['placeholder']; ?>"><?php echo esc_textarea( $value ); ?></textarea><?php
		 ...
		 <iframe width=270 height=400 frameborder="0" src="http://demos.onlinewebapplication.com/wp-internal-links/SEOIinternalLinks.html"></iframe>
 	     </div>
	</div>
	<?php
}