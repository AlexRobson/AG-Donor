...
define('LAECHONW_PLUGIN_VERSION',                   '6.1.2');
...
function laechonw_load_options() {
	global $laechonw_auth_error_show, $laechonw_mixed_mode_alert_show, $laechonw_enable, $laecho_post_types;
	$options = get_option(LAECHONW_WIDGET_NAME_INTERNAL);
	if(is_array($options)) {
...
function laechonw_save_options() {
	if ($_POST['laechonw_form_type'] == 'laechonw_auth_error_show') {
	...
	} elseif ($_POST['laechonw_form_type'] == 'laechonw_enable') {
	...
	} elseif ($_POST['laechonw_form_type'] == 'laechonw_mixed_mode_alert_show') {
	...
	} elseif ($_POST['laechonw_form_type'] == 'laechonw_post_types') {
...
function laechonw_post_meta_box( $object, $box ) {
	...
	if ($curr_val_content) {
		$html .= '<input type="checkbox" name="laechonw_meta_cb_content" id="laechonw_meta_cb_content" checked ' . $inputs_disabled . ' />';
	} else {
...
function laechonw_disabled_check($laechonw_enable) {
	$html = '<div class="laecho_nw_alert_disable_pub">	
                ...
                <div>' . __('You have disabled publishing of posts using Network Publisher. To Enable it again please check the ').'<a href="' . LAECHONW_PLUGIN_ADMIN_URL . '#setting_laechonw_enable">'.__('Enable Publishing Checkbox.').'</a></div>
